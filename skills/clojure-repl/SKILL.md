...
(map my-function [input1 input2 input3])

;; Test error handling
(try
  (my-function invalid-input)
  (catch Exception e
    {:error (.getMessage e)}))
```
...
;; 2. Build transformation functions
...
;; 4. Test edge cases
(user-summary {})
(user-summary {:name "Bob"})
```

### Pattern 2: Bottom-Up Function Building

```clojure
;; 1. Start with smallest pieces
(defn extract-domain [email]
  (second (clojure.string/split email #"@")))

(extract-domain "alice@example.com")
;; => "example.com"

;; 2. Build next layer
(defn email-summary [user]
  {:name (:name user)
   :domain (extract-domain (:email user))})

(email-summary user)
;; => {:name "Alice", :domain "example.com"}

;; 3. Combine into final solution
(defn user-report [users]
  (map email-summary users))
```

### Pattern 3: Exploratory Testing

```clojure
;; 1. Explore API or library
...
;; 4. Build confident implementation
(defn parse-csv-line [line]
  (remove empty? (str/split line #",")))
```

### Pattern 4: Debugging with REPL

```clojure
;; 1. Reproduce the problem
(def problematic-input {:data [1 2 nil 4]})

;; 2. Isolate the issue
(map inc (:data problematic-input))
;; Error: NullPointerException

;; 3. Test fix
(map #(when % (inc %)) (:data problematic-input))
;; => (2 3 nil 5)

;; 4. Better fix
(keep #(when % (inc %)) (:data problematic-input))
;; => (2 3 5)
...
(take 5 large-collection)
(first collection)
(last collection)
```

### Function Composition Experimentation

```clojure
;; Try different compositions
(def process-v1 (comp format-output transform validate))
(process-v1 sample-data)

(def process-v2 (comp format-output enrich validate))
(process-v2 sample-data)

;; Compare results
(= (process-v1 sample-data) (process-v2 sample-data))
```
...
Complete example of REPL-driven development session:

```clojure
;; Problem: Need to parse CSV data and extract user information

;; 1. Define sample data
(def csv-line "Alice,30,alice@example.com")
...
;; 3. Build structure
(defn parse-user-csv [line]
  (let [[name age email] (clojure.string/split line #",")]
    {:name name
     :age age
     :email email}))

(parse-user-csv csv-line)
;; => {:name "Alice", :age "30", :email "alice@example.com"}

;; 4. Notice age is string, fix it
(defn parse-user-csv [line]
  (let [[name age email] (clojure.string/split line #",")]
    {:name name
...
;; 5. Test edge cases
(def bad-line "Bob,invalid,bob@example.com")
(parse-user-csv bad-line)
  ...
  (try
    (let [[name age email] (clojure.string/split line #",")]
      {:name name
       ...
       :email email})
    (catch Exception e
      {:error (.getMessage e) :line line})))

(parse-user-csv bad-line)
;; => {:error "...", :line "Bob,invalid,bob@example.com"}

;; 7. Working! Save to file using clojure-edit skill

;; 8. Reload and test
(require '[my.csv :as csv] :reload)
(csv/parse-user-csv csv-line)
;; => {:name "Alice", :age 30, :email "alice@example.com"}
...
For additional REPL techniques and development workflows, see:
- `references/repl_workflows.md` - Detailed workflow patterns and examples
